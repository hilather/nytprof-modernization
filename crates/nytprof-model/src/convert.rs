...
//! - No `--allow-lossy` mode in this MVP (explicit residual)
...
pub fn convert_bytes(input: &[u8], target: ConvertTarget) -> ConvertResult<Vec<u8>> {
    ...
    encode_events(&events, target)
}

/// Convert a profile file and write the result to `output_path`.
pub fn convert_path(
    ...
    let out = convert_bytes(&bytes, target)?;
...
fn encode_to_v5(events: &[Event]) -> ConvertResult<Vec<u8>> {
    ...
    nytprof_format_v5::encode_all_as_v5(events)
        .map_err(|e| ConvertError::V5Encode(e.to_string()))
}

/// Build owned string storage + EventRecordSpec list, then absolute v6 encode.
fn encode_to_v6(events: &[Event]) -> ConvertResult<Vec<u8>> {
    ...
    enum Op {
        ...
        Comment { si: usize },
        Attribute { ki: usize, vi: usize },
        Option { ki: usize, vi: usize },
        StartDeflate,
        PidStart {
            pid: u64,
            ppid: u64,
    ...
    for ev in events {
        match ev.tag.as_str() {
            ...
            tags::PID_START => {
                ...
                let start_time = arg_exact_u64_ticks(&ev.args, 2, "PID_START", "start_time", ev.seq)?;
            ...
            tags::PID_END => {
                ...
                let end_time = arg_exact_u64_ticks(&ev.args, 1, "PID_END", "end_time", ev.seq)?;
            ...
            tags::TIME_LINE => {
                let ticks = arg_exact_u64_ticks(&ev.args, 0, "TIME_LINE", "ticks", ev.seq)?;
            ...
            tags::TIME_BLOCK => {
                let ticks = arg_exact_u64_ticks(&ev.args, 0, "TIME_BLOCK", "ticks", ev.seq)?;
            ...
            tags::SUB_RETURN => {
                ...
                let incl = arg_exact_u64_ticks(&ev.args, 1, "SUB_RETURN", "incl", ev.seq)?;
                let excl = arg_exact_u64_ticks(&ev.args, 2, "SUB_RETURN", "excl", ev.seq)?;
            ...
            tags::SUB_CALLERS => {
                ...
                let incl = arg_exact_u64_ticks(&ev.args, 3, "SUB_CALLERS", "incl", ev.seq)?;
                let excl = arg_exact_u64_ticks(&ev.args, 4, "SUB_CALLERS", "excl", ev.seq)?;
                let reci = arg_exact_u64_ticks(&ev.args, 5, "SUB_CALLERS", "reci", ev.seq)?;
            ...
            other => {
                return Err(ConvertError::Strict {
                    detail: format!(
                        "seq {}: tag '{other}' has no strict v6 mapping",
                        ev.seq
                    ),
                });
            }
    ...
    e3_standin_write_absolute(&specs, codec::NONE).map_err(|e| ConvertError::V6Encode(e.to_string()))
}
        ...
        Value::Number(n) => {
            ...
            if let Some(i64v) = n.as_i64() {
                if i64v < 0 {
                    return Err(ConvertError::Strict {
                        detail: format!(
                            "seq {seq}: {tag}.{field}={i64v} negative (strict refuse)"
                        ),
                    });
                }
