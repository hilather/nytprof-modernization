...
| **COL-001..005 + fake-clock scaffold (landed)** | Offline gate step **10** runs `./scripts/packaging/collector_sink_smoke.sh`: isolation asserts always; `make -C collector test` (sink + lifecycle/seq + M4 mini + **batch/fast**) when CC present; **honest skip** without C toolchain. Stub v5 adapter is **not** wire-byte neutrality vs oracle (**COL-006**); full M4 oracle corpus under fake-clock remains residual (COL-006 + complete TEST-003); light microbench is **not** BENCH certification; flush-discount timing residual |
| **COL-006** | Expand to oracle-aligned v5-via-sink stream equality on the agreed corpus once the real writer is adapted |
...
| Collector overlay sources / COL-001..005 + fake-clock scaffold | **partial (PR-B02..B04)** | `collector/` headers + counting/stub-v5 sinks + lifecycle/seq + **bounded batch + stmt fast path** + fake-clock mini M4 + unit tests + `collector_sink_smoke.sh` in offline_gate step 10; **not** live XS hooks, real v5 wire (COL-006), full M4 corpus gate, BENCH cert, or COL-007 |
