...
**Residual honesty (from REPORT_SURFACE_CONTRACT):** native HTML is an MVP summary + multi-file site (including MVP shared `style.css` / documented inline CSS policy and **`index-subs-excl.html`** exclusive ranking — **not** oracle CSS/JS/DOM). Shared JS / tablesorter is **WAIVE** for GA-candidate (**PR-M01** / Q4 user-final — documentation residual, **not** shipped). Flame (A03 **OPEN**), Graphviz, block/sub-level oracle page modes, and full DOM are **not advertised**.
...
| Shared JS (jquery / tablesorter / floatThead) | **yes** — `js/jquery-min.js`, `js/jquery.tablesorter.min.js`, `js/style-tablesorter.css`, sort icons `js/asc.png` `js/bg.png` `js/desc.png`; HTML also references `js/jquery.floatThead.min.js` when jquery headers emit | **no** | **yes** | **WAIVE** — **PR-M01** / Q4 user-final: GA-candidate does **not** require shipping jquery/tablesorter/floatThead (documentation residual, not CLOSE). ADR-0003 originally mapped CLOSE via **PR-A01**; A01 closed **CSS/structure only**. jquery **not** shipped. Native HTML remains MVP, not oracle DOM. Amendment: [`0003-r1-full-residual-policy.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/adrs/0003-r1-full-residual-policy.md) |
...
| Flame graph SVG | **yes** when `--flame` (default) and `calls` option on — `all_stacks_by_time.svg` via bundled `flamegraph.pl` + `nytprofcalls` | **no** (folded export is a separate CLI for external flame tools; not embedded SVG site) | **yes** | **CLOSE** via **PR-A03** (optional `--flame`; no default bloat) |
| Call-stack flame inputs | **yes** — `all_stacks_by_time.calls`, `flamegraph_subattr.txt` | **no** as HTML site artifacts; native `folded` CLI is a related operator export | **yes** (site) / related export **mapped** outside HTML | **CLOSE** site inputs with flame via **PR-A03**; `folded` remains related export |
| Packages call graph (Graphviz) | **yes** non-minimal — `packages-callgraph.dot` | **no** | **yes** | **WAIVE** (waive-by-default) |
| Subs call graph (Graphviz) | **yes** non-minimal — `subs-callgraph.dot` | **no** | **yes** | **WAIVE** (waive-by-default) |
...
4. **Largest residual classes** vs oracle site: shared **JS/tablesorter** (**WAIVE** for GA-candidate — **PR-M01** / Q4; not shipped), **flame SVG** + call-stack inputs (A03 **OPEN**), **Graphviz** (`packages-callgraph.dot`, `subs-callgraph.dot`, per-file `.dot`), **treemap**, **block/sub page modes** (`*-block.html` / `*-sub.html`).  
...
8. **Full R1 disposition (ADR-0003 / PR-A04 + PR-A10 cut + PR-M01 amendment):** **closed (MVP)** A01 CSS/structure + A02 excl index. **OPEN residual (CLOSE path):** A03 optional flame. **WAIVE** Shared JS/tablesorter (**PR-M01** / Q4 user-final — documentation residual, not CLOSE; jquery **not** shipped); Graphviz, treemap, block/sub page modes, oracle per-file naming, browser `--open`, exact `-d`, mergeevals, oracle footer. PR-A10 / GA-candidate must **not** claim full oracle DOM — see matrix § Full R1 ready + [`docs/RELEASE_NOTES_R1.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/RELEASE_NOTES_R1.md).
...
| `M01-HTML-JS-WAIVE` | **done (docs)** (PR-M01) | Shared JS/tablesorter **WAIVE** for GA-candidate (Q4 user-final). Documentation residual only — **not** a jquery/tablesorter implementation. Flame A03 remains **OPEN**. |
