...
| Flame graph SVG | **yes** when `--flame` (default) and `calls` option on — `all_stacks_by_time.svg` via bundled `flamegraph.pl` + `nytprofcalls` | **yes** opt-in `--flame` — native call-tree `all_stacks_by_time.svg` from `call_edges` (inclusive-time widths; click-to-source; hover details; not `flamegraph.pl`) | **partial** | **closed (MVP)** via **PR-A03** + tree paint + inlined index SVG (hover + `file-*.html#L` links) |
| Call-stack flame inputs | **yes** — `all_stacks_by_time.calls`, `flamegraph_subattr.txt` | **yes** opt-in — `all_stacks_by_time.folded` (`render_folded_stacks`); not `nytprofcalls` | **partial** | **closed (MVP)** site inputs with `--flame`; `folded` CLI remains related export |
...
4. **Largest residual classes** vs oracle site: shared **JS/tablesorter** (**WAIVE** for GA-candidate — **PR-M01** / Q4; not shipped), Graphviz **PNG/SVG** (`.dot` source is **closed (MVP)**), **treemap**, **block/sub page modes** (`*-block.html` / `*-sub.html`). Native flame is **opt-in `--flame`** (call-tree SVG from `call_edges`; **not** `flamegraph.pl`).  
...
8. **Full R1 disposition (ADR-0003 / PR-A04 + PR-A10 cut + PR-M01 amendment):** **closed (MVP)** A01 CSS/structure + A02 excl index + native Graphviz `.dot` source + **A03 optional `--flame`** (call-tree SVG from `call_edges`; **not** oracle default-on / `flamegraph.pl` / `nytprofcalls`). **WAIVE** Shared JS/tablesorter (**PR-M01** / Q4 user-final — documentation residual, not CLOSE; jquery **not** shipped); Graphviz image render, treemap, block/sub page modes, oracle per-file naming, browser `--open`, exact `-d`, mergeevals, oracle footer. PR-A10 / GA-candidate must **not** claim full oracle DOM — see matrix § Full R1 ready + [`docs/RELEASE_NOTES_R1.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/RELEASE_NOTES_R1.md).
...
| `M01-HTML-JS-WAIVE` | **done (docs)** (PR-M01) | Shared JS/tablesorter **WAIVE** for GA-candidate (Q4 user-final). Documentation residual only — **not** a jquery/tablesorter implementation. Flame A03 is **closed (MVP)** opt-in `--flame` (not `flamegraph.pl`). |
