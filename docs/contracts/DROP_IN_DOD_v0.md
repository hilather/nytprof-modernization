...
**Status:** docs-landed (PR-G01) — G03a load + G03b–G03e emit-MVP + G04 attach-MVP + G05 options/`format=v6` + G06 fork/`addpid` MVP landed; E1b default call attach is opcode `OP_ENTERSUB` (wrap is `wrap=1`); **not** full TEST-018 / mid-deflate-in-child / DI-03 done (E2–E4 + live di02 **21** vs oracle **27**)  

**Board ID:** `DROP-IN-DOD-V0`  
**Date:** 2026-08-12 (E1b attach honesty 2026-08-17)  
...
| `use_db_sub` | 6.15: stmt `DB::DB` + opcode calls still on | **parse/stamp 0/1** (E0) | wrap-escape **fork** | **Not** 6.15 stmt `DB::DB`. Product synonym for `wrap=1`. Values other than 0/1 fail-closed. |
| `wrap` | **new product** | **parse/stamp 0/1** (E0) | wrap escape (E1b) | Canonical wrap escape. Wins over default/`entersub=1`. Values other than 0/1 fail-closed. |
| `entersub` | **new product** | **parse/stamp 0/1** (E0) | E1b default opcode | Omit / `=1` installs `OP_ENTERSUB`. `entersub=0` forces wrap. `wrap=1` / `use_db_sub=1` wins. Values other than 0/1 fail-closed. **DI-03 opcode/`entersub` — in progress, not done** (E2 GOTO residual). |
...
| `DROP-IN-REMAINING` | Remaining advertised-options / GA drop-in | D2 + S2 + publish + DI-03 E2–E4 | **residual** — E1b default `OP_ENTERSUB` landed; wrap is `wrap=1`. DI-03 **not** done. Completion design: [DROP_IN_RPM_COMPLETION_v0.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/DROP_IN_RPM_COMPLETION_v0.md) |

**Not ready (do not advertise as shipped):** DI-03 done (E2 `OP_GOTO` / E3 leave default 0 / E4 full slowops / live di02 **21** vs oracle **27** remain), DI-05 / full TEST-003, GA-candidate, certified perf, full TEST-018, mid-deflate-in-child, signed CI prebuilt **pipeline**, PAUSE upload, `BUILD-003-FULL`, independent SEC-012 sign-off, full SEC-002 cargo-fuzz/AFL. `EL8-RPM-MODULE` / `EL8-RPM-TOOLS` are **spec MVP** only (not mock-certified / not pipeline-complete). `CPAN-TRIAL-READY` is **notes-ready MVP** only (not uploaded). P02 is **checklist / job MVP** only (not GA marketing). G03a debugger **load**, G03b–G03e **emit-MVP**, G04 **attach-MVP** (E1b default opcode), G05 **options/`format=v6` tests**, and G06 **fork/`addpid` MVP** are shipped.
