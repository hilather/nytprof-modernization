...
| Strict v5↔v6 convert tooling | **TOOL-CONVERT-STRICT-MVP** **done** (PR-C01) | `nytprof-cli convert --to=v5\|v6`; `nytprof_model::convert_*`; capability `convert: yes`; schema [`docs/schemas/convert-strict-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/convert-strict-mvp-v0.md); refuse non-zero `sub_line` / extended NEW_FID / non-exact f64 NV | No lossy mode; packing/dict v6 out; merge/salvage (PR-C02); oracle wall NV PID refuse; blocks dual-sink v5→v6 residual |
