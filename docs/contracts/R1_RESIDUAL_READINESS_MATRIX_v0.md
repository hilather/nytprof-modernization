...
| `PRODUCT-XS-ATTACH-MVP` | **done (MVP)** — live `-d:NYTProf` + `NYTPROF file=`; E1b default `OP_ENTERSUB` (`$^P` 0x01 off). dump/report leaf **15** / mid **3** / mid→leaf **15**. **Residuals:** E2 GOTO / E3 leave / E4 full slowops |
...
| `DROP-IN-REMAINING` | **residual** — opcode/full TEST-003/S2/COMPAT-007/publish. **DI-01 780/810**, **DI-02 27+CORE:**, **DI-04 mini projected kinds** landed. **DI-03 opcode/`entersub` — in progress, not done** (E1b default omit installs `OP_ENTERSUB`; `wrap=1` / `use_db_sub=1` / `entersub=0` escape; E2 GOTO residual). |
...
| `PRODUCT-XS-ATTACH-MVP` | **done (MVP)** | Live `-d:NYTProf` + `file=`; E1b default `OP_ENTERSUB` (`$^P` 0x01 off). wrap remains `wrap=1`. Smoke [`g04_v5_parity_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g04_v5_parity_smoke.sh) leaf **15** / mid **3** / edge **15**. **Not** E2 GOTO / E3 leave / E4 full slowops. |
...
| `PRODUCT-NESTED-EXCL-MVP` | **done (MVP)** | PR-14: exclusive = incl − Σ child inclusive; `stmts=0` skips TIME_LINE. Smoke [`g14_nested_excl_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g14_nested_excl_smoke.sh); g17 re-drives on default opcode. DateTime/Moo `goto` still untimed as own rows (E2). |
...
| `PRODUCT-WRAP-ENTER-MVP` | **done (MVP)** | PR-16 + E1b: `wrap=1` escape `wrap_push`/`wrap_pop`; `WRAP_SLOW` nested under that escape. Default attach is opcode (g17). Smoke [`g16_wrap_enter_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g16_wrap_enter_smoke.sh). **Not** E2 GOTO / stock 6.15 XS. |
...
| `PRODUCT-XS-ATTACH-MVP` | **done (MVP)** | Live `-d:NYTProf` default-calls1 **15/3/15**; E1b default `OP_ENTERSUB`. **Not** E2 GOTO / E3 leave / E4 full slowops |
