...
|----------|-------------------|--------------|-----------------|--------------------------------|
| No production C ABI / FFI / cdylib | **RUST-010**, `nytprof-ffi` (charter crate list only) | No shipped stable native library ABI for embedders; Perl bridge is **subprocess CLI**, not FFI | Pure-Rust crates + CLI only | **CLOSE** via **PR-A05** (**OQ-2** — waiver forbidden) |
...
2. Residual table still lists **no production FFI/XS Data** (RUST-010, PERL-004/005), **no full nytprofhtml DOM** (REPORT-001..020 + HTML residual inventory), **no v6 / COL-007** (COL-007/008), **no multi-OS CI** (BUILD-006), **no perf claims**, R3 default flip, and Rust CLI `auto`→native residual — **preview honesty unchanged**. Full-R1 **disposition** is ADR-0003: FFI/XS → **CLOSE** PR-A05/A06 (**OQ-2**); HTML → per-class map; BUILD-003/006 → preferred CLOSE A08/A07; perf → CLOSE A09 or WAIVE public claims.
