...
| Full nytprofhtml DOM / REPORT-001..020 | **REPORT-001..020**, **REPORT-HTML-RESIDUAL-INV**, **REPORT-HTML-SHARED-CSS**, **REPORT-HTML-SUBS-EXCL** | **Partial CLOSE + WAIVE** per ADR-0003 HTML map — **not** full oracle DOM. **CLOSED (MVP):** A01 shared CSS/structure (`style.css` / inline policy); A02 exclusive sub index (`index-subs-excl.html`). **OPEN residual (CLOSE path):** A03 optional flame SVG + call-stack site inputs (not shipped on this cut; native `folded` remains related export). **WAIVE** (residual-honest): Graphviz, treemap, JIT, block/sub page modes, oracle per-file naming, Shared JS/tablesorter beyond A01 structure, browser `--open`, exact `-d`, mergeevals, oracle footer | Inventory: [`REPORT_HTML_RESIDUAL_INVENTORY_v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/contracts/REPORT_HTML_RESIDUAL_INVENTORY_v0.md). **Do not claim full oracle `nytprofhtml` DOM** |
...
| `engine=auto` product default flip | charter **R3** | **OUT-OF-R1** | Perl facade prefer-native/fallback shipped; **not** product default. Rust CLI `auto`→`native` |
| Default engine/format flips | charter R3/R4 | **OUT-OF-R1** | Native remains opt-in |
...
| **Full R1 ready (MVP product scope)** | **yes (scoped)** | ADR-0003 Phase A product cut: OQ-2 FFI/XS **MVP closed**; HTML **A01/A02 closed**, **A03 flame residual open**, WAIVE classes residual-honest; **BUILD-006-MVP** + **BUILD-003-DEPTH** closed; public perf **WAIVED**. See non-claims |
...
| PR | Role | Cut status |
|----|------|------------|
| **PR-A01** | Shared CSS + structure | **closed (MVP)** — not Shared JS/tablesorter/oracle CSS |
...
| **Not claimed** | Full multi-OS product certification; CPAN upload; performance SLOs; v6 collection / COL-007 encoder; full RUST-010 ABI freeze / production dylib install; full PERL-004/005 pure-XS / COMPAT-007; full oracle `nytprofhtml` DOM; native flame site (A03) |
