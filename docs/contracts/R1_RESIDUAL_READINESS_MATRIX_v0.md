...
**Full R1 close-or-waive policy (binding):**  
...
Every residual below has disposition **CLOSE** (named Phase A PR), **WAIVE**, or **OUT-OF-R1**. **OQ-2 (resolved):** FFI and XS Data/ReadStream map to **CLOSE** (PR-A05 / PR-A06) — **not** waive. HTML classes are mapped per-class in the HTML inventory + ADR-0003 (PR-A10 must not claim full R1 HTML without that map).

| Residual | Plan / board refs | Why residual | Preview honesty | Full R1 disposition (ADR-0003) |
|----------|-------------------|--------------|-----------------|--------------------------------|
| Full RUST-010 C ABI (beyond open/query/close MVP) | **RUST-010**, **FFI-CDYLIB-MVP** / **PR-A05** | **MVP shipped:** `crates/nytprof-ffi` cdylib + C header open/query/close over `ProfileModel` (panic-safe; schema [`docs/schemas/ffi-cdylib-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/ffi-cdylib-mvp-v0.md)). **Still residual for full RUST-010:** batch/event-walk APIs, BUILD-007 header automation, production dylib install (BUILD-004), sanitizer/Miri package, ABI freeze tooling | Preview primary path remains **CLI subprocess** + pure-Perl JsonlData; dylib is **opt-in embedder product path** (dual-path must work without loading it). Not a full ABI stability / packaging claim | **CLOSE path in progress** via **PR-A05** (**OQ-2** — waiver forbidden). MVP = partial product close; full row closes when RUST-010 completeness items land (or superseding ADR scopes them). **Do not waive.** |
| XS ReadStream over binary profiles (PERL-004) | **PERL-004**, **PERL-XS-DATA-READSTREAM-MVP** / **PR-A06** | **MVP shipped:** product `Devel::NYTProf::ReadStream` opens **binary** profiles via native `nytprof-cli dump` → `JsonlReadStream` (thin materializer; schema [`docs/schemas/perl-xs-data-readstream-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/perl-xs-data-readstream-mvp-v0.md)). Dump-JSONL bridge still available. **Still residual for full PERL-004:** pure-XS wire decode (no CLI), full scalar-flag package, dual-engine callback fidelity | Product path is thin native-cli-jsonl; pure-Perl `JsonlReadStream` remains the dump-JSONL bridge. Not pure-XS binary decoder | **CLOSE path in progress** via **PR-A06** (**OQ-2** — waiver forbidden). MVP = partial product close. **Do not waive.** |
| XS / bless-array Data materializer (PERL-005) | **PERL-005**, **PERL-XS-DATA-READSTREAM-MVP** / **PR-A06** (+ COMPAT-007) | **MVP shipped:** product `Devel::NYTProf::Data` opens **binary** profiles via native dump → `JsonlData` query surface (`from_profile` / `new({filename})`; same schema). `claims_compat007_shapes` always **0**. **Still residual for full PERL-005:** COMPAT-007 bless-array / AV-HV fidelity, full oracle Data method set, in-process FFI-backed materializer | Preview dump-JSONL `JsonlData` unchanged; product facade is the binary-capable Data path. Not full `Devel::NYTProf::Data` oracle fidelity | **CLOSE path in progress** via **PR-A06** (**OQ-2** — waiver forbidden; COMPAT-007 only with tests). MVP = partial product close. **Do not waive.** |
| No full nytprofhtml DOM / REPORT-001..020 | **REPORT-001..020**, BASE-005, **REPORT-HTML-RESIDUAL-INV** | Native HTML is MVP summary + multi-file site; not oracle DOM/CSS/tablesorter/flame/Graphviz | REPORT_SURFACE_CONTRACT **not advertised** list; **artifact residual matrix:** [`docs/contracts/REPORT_HTML_RESIDUAL_INVENTORY_v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/contracts/REPORT_HTML_RESIDUAL_INVENTORY_v0.md) (oracle vs native classes on default-calls1) | **Per-class CLOSE/WAIVE** map in inventory + ADR-0003 (CSS/JS → PR-A01; excl index → PR-A02; flame → PR-A03; Graphviz/treemap/block-sub/naming → **WAIVE** by default). PR-A10 cannot claim full R1 HTML without the map. |
| No v6 wire freeze | format plan / charter Phase-0 | Event contract provisional; no stable v6 numeric/wire IDs | v5 read/report path only | **OUT-OF-R1** (R2 wire freeze after E3/E4) |
| COL-007 / COL-008 deferred | **COL-007** board deferred; **COL-008** non-baseline | C v6 writer after report-side evidence; batched Rust writer deferred until dual-equality + ADR. **Preflight only:** provisional fixed-header + chunk-frame + ULEB128 + ZigZag signed + length-prefixed string + header TLV + multi-TLV region + file-prefix + prefix+chunk stream + event-body (incl. TIME_BLOCK/SUB_ENTRY/SUB_RETURN/SUB_INFO/SRC_LINE/NEW_FID/PID_START/PID_END/SUB_CALLERS/DISCOUNT/ATTRIBUTE/OPTION/COMMENT/START_DEFLATE/VERSION/dual-output-sequence/mid-stream-codec-switch/auto-emit-VERSION/known-key-attr-option/unknown-optional-skip) + mini-profile + multi-chunk EVENT + SOURCE/INDEX/SUMMARY/FOOTER bodies + CRC32 optional verify + ZLIB/ZSTD/LZ4 payload codecs + compressed multi-codec mini-profile + multi-chunk compressed EVENT + compressed multi-kind mixed + per-kind codecs + multi-chunk EVENT under mixed + multi-chunk SOURCE + multi-chunk INDEX + multi-chunk SUMMARY + mid-record EVENT/SOURCE/INDEX/SUMMARY span + decoded-chunk + decoded-stream + decoded-EVENT/SOURCE/INDEX/SUMMARY + decoded-mixed + multi-chunk decoded-mixed + mid-record decoded-mixed (EVENT+SOURCE+INDEX+SUMMARY + concurrent multi-kind) always-inflate (`FMT-V6-HEADER-*` / `FMT-V6-CHUNK-*` / `FMT-V6-VARINT-*` / `FMT-V6-SVARINT-*` / `FMT-V6-STRING-*` / `FMT-V6-TLV-*` / `FMT-V6-TLV-REGION-*` / `FMT-V6-FILE-PREFIX-*` / `FMT-V6-PREFIX-CHUNK-STREAM-*` / `FMT-V6-EVENT-BODY-*` / `FMT-V6-MINI-PROFILE-*` / `FMT-V6-MULTI-CHUNK-EVENT-*` / `FMT-V6-SOURCE-BODY-*` / `FMT-V6-INDEX-BODY-*` / `FMT-V6-SUMMARY-BODY-*` / `FMT-V6-FOOTER-BODY-*` / `FMT-V6-CRC-*` / `FMT-V6-PAYLOAD-ZLIB-*` / `FMT-V6-PAYLOAD-ZSTD-*` / `FMT-V6-PAYLOAD-LZ4-*` / `FMT-V6-COMPRESSED-PROFILE-*` / `FMT-V6-MULTI-CHUNK-COMPRESSED-*` / `FMT-V6-COMPRESSED-MIXED-*` / `FMT-V6-PER-KIND-CODEC-*` / `FMT-V6-MULTI-CHUNK-KIND-*` / `FMT-V6-MULTI-CHUNK-SOURCE-*` / `FMT-V6-MULTI-CHUNK-INDEX-*` / `FMT-V6-MULTI-CHUNK-SUMMARY-*` / `FMT-V6-MID-RECORD-SPAN-*` / `FMT-V6-MID-RECORD-SOURCE-*` / `FMT-V6-MID-RECORD-INDEX-*` / `FMT-V6-MID-RECORD-SUMMARY-*` / `FMT-V6-DECODED-CHUNK-*` / `FMT-V6-DECODED-STREAM-*` / `FMT-V6-DECODED-EVENT-*` / `FMT-V6-DECODED-SOURCE-*` / `FMT-V6-DECODED-INDEX-*` / `FMT-V6-DECODED-SUMMARY-*` / `FMT-V6-DECODED-MIXED-*` / `FMT-V6-DECODED-MIXED-MULTI-CHUNK-*` / `FMT-V6-DECODED-MIXED-MID-RECORD-*` / `FMT-V6-DECODED-MIXED-MID-RECORD-SOURCE-*` / `FMT-V6-DECODED-MIXED-MID-RECORD-INDEX-*` / `FMT-V6-DECODED-MIXED-MID-RECORD-SUMMARY-*` / `FMT-V6-DECODED-MIXED-MID-RECORD-CONCURRENT-*` / `FMT-V6-EVENT-BODY-TIME-BLOCK-SUB-ENTRY-*` / `FMT-V6-EVENT-BODY-SUB-RETURN-SUB-INFO-*` / `FMT-V6-EVENT-BODY-SRC-LINE-NEW-FID-*` / `FMT-V6-EVENT-BODY-PID-START-END-*` / `FMT-V6-EVENT-BODY-SUB-CALLERS-DISCOUNT-*` / `FMT-V6-EVENT-BODY-ATTRIBUTE-OPTION-*` / `FMT-V6-EVENT-BODY-COMMENT-*` / `FMT-V6-EVENT-BODY-START-DEFLATE-*` / `FMT-V6-EVENT-BODY-VERSION-*`; schemas under `docs/schemas/v6-*-provisional-v0.md`; crate `nytprof-format-v6`) — **not** full COL-007 encoder / dictionaries / always-on inflate / CLI v6 default | Collector remains 6.15 oracle / v5; no default v6 CLI report | **OUT-OF-R1** (R2 COL-007 critical path; preflight ≠ product writer) |
| No full MakeMaker XS dual-build CPAN | **BUILD-003** (full) | Candidate `Makefile.PL` facade only (**BUILD-MAKEMAKER-OPT** done) | Not a complete XS CPAN tarball dual-build | **CLOSE** (preferred) via **PR-A08**; future waiver only via superseding ADR |
| No multi-OS CI matrix | **BUILD-006** | Single-host offline gate only | `offline_gate.sh` is not multi-OS CI | **CLOSE** (preferred) via **PR-A07** (exact matrix **OQ-3** open); future waiver only via superseding ADR |
| No performance certification claims | WP-13 / BENCH-001 | Light wall-time notes only | `docs/BENCH_NOTES.md`, `tools/bench/light_bench.sh` — **no public perf claims** | **CLOSE or WAIVE**: **PR-A09** if publishing R1 P3/P4 claims; else **WAIVE** public claims (default full-R1 posture) |
| `engine=auto` full product policy / default flip | charter **R3**; ENGINE-AUTO-FALLBACK done for **Perl facade** | Perl `nytprof-engine`: prefer-native / fall-back-legacy is shipped. Residual: R3 product **default** flip + field window/ADR; pure-Rust `nytprof-cli` still maps `auto`→`native` (no in-process legacy) | Facade smokes prove dual-path auto; **not** “auto is the product default” | **OUT-OF-R1** (Phase D / R3) |
| Default engine/format flips | charter R3/R4 | Explicitly out of first slice | Native remains opt-in | **OUT-OF-R1** (Phase D/E) |
...
| **Not claimed** | Multi-OS CI green matrix, CPAN upload, performance SLOs, v6 collection, full RUST-010 ABI freeze / production dylib install (FFI open/query/close **MVP** may be present under `FFI-CDYLIB-MVP` without those claims), full PERL-004/005 pure-XS / COMPAT-007 bless-array (thin product path **MVP** may be present under `PERL-XS-DATA-READSTREAM-MVP` without those claims) — until matching completeness lands and honesty docs flip |
...
2. Residual table still lists **full RUST-010 beyond FFI MVP** + **full PERL-004/005 beyond thin product MVP** (`PERL-XS-DATA-READSTREAM-MVP` may be present without COMPAT-007 / pure-XS claims), **no full nytprofhtml DOM** (REPORT-001..020 + HTML residual inventory), **no v6 / COL-007** (COL-007/008), **no multi-OS CI** (BUILD-006), **no perf claims**, R3 default flip, and Rust CLI `auto`→native residual — **preview honesty**: FFI open/query/close MVP (`FFI-CDYLIB-MVP`) + XS Data/ReadStream thin product path MVP may be present without full ABI freeze/install or COMPAT-007. Full-R1 **disposition** is ADR-0003: FFI/XS → **CLOSE** PR-A05/A06 (**OQ-2**); HTML → per-class map; BUILD-003/006 → preferred CLOSE A08/A07; perf → CLOSE A09 or WAIVE public claims.
...
| `R1-HONESTY-SYNC` | **done** | this matrix + runbook re-synced to advertise **NATIVE-QUERY-JSON-CROSS** / **CROSS-EXPAND** / **CROSS-BLOCKS** / **CROSS-META** / **CROSS-TIMEBLOCK** / **CROSS-TOTAL** / **CROSS-COUNTS**, **JSON-EVENT-COUNTS-MVP**, **JSON-FILE-BASENAME-MVP** (absolute `file_1` volatile; basename greppable stable sample), **JSON-TIME-BLOCK-MVP**, **JSON-REPORT-INCOMPLETE-FAILCLOSED**, **JSON-SUB-ENTRY-MVP**, **JSON-BLOCKS-MVP**, **JSON-META-FILES-MVP**, + **PERL-SUB-ENTRY-JSONL** + **FFI-CDYLIB-MVP** (PR-A05) + **PERL-XS-DATA-READSTREAM-MVP** (PR-A06 thin binary Data/ReadStream) while retaining full-R1 residual honesty (full RUST-010 beyond MVP, full PERL-004/005 beyond thin product path / no COMPAT-007, no full nytprofhtml DOM, no v6/COL-007, no multi-OS CI, no perf claims). Evidence: `scripts/packaging/native_query_json_cross_smoke.sh`, `scripts/packaging/json_time_block_smoke.sh`, `scripts/packaging/json_report_incomplete_smoke.sh`, `scripts/packaging/perl_sub_entry_smoke.sh`, `scripts/packaging/perl_xs_data_readstream_smoke.sh`, `perl/t/jsonl_data_sub_entry.t`, `crates/nytprof-cli/tests/native_agg_json.rs`, `cargo test -p nytprof-ffi`, offline_gate steps 1 + 5b + 6f–8 when native |
