...
| No XS ReadStream over binary profiles | **PERL-004** | Preview is dump-JSONL pure-Perl `JsonlReadStream` only | [`perl-jsonl-readstream-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/perl-jsonl-readstream-mvp-v0.md) non-goal | **CLOSE** via **PR-A06** (**OQ-2** — waiver forbidden) |
| No XS / bless-array Data materializer | **PERL-005** (+ COMPAT-007 shapes) | Preview is pure-Perl `JsonlData` query subset from dump JSONL | Not full `Devel::NYTProf::Data` fidelity | **CLOSE** via **PR-A06** (**OQ-2** — waiver forbidden; COMPAT-007 only with tests) |
...
| **Not claimed** | Multi-OS CI green matrix, CPAN upload, performance SLOs, v6 collection, full RUST-010 ABI freeze / production dylib install (FFI open/query/close **MVP** may be present under `FFI-CDYLIB-MVP` without those claims) — until the matching CLOSE PR lands and honesty docs flip |
...
2. Residual table still lists **full RUST-010 beyond FFI MVP** + **no XS Data** (PERL-004/005), **no full nytprofhtml DOM** (REPORT-001..020 + HTML residual inventory), **no v6 / COL-007** (COL-007/008), **no multi-OS CI** (BUILD-006), **no perf claims**, R3 default flip, and Rust CLI `auto`→native residual — **preview honesty**: FFI open/query/close MVP may be present (`FFI-CDYLIB-MVP`) without claiming full ABI freeze/install. Full-R1 **disposition** is ADR-0003: FFI/XS → **CLOSE** PR-A05/A06 (**OQ-2**); HTML → per-class map; BUILD-003/006 → preferred CLOSE A08/A07; perf → CLOSE A09 or WAIVE public claims.
