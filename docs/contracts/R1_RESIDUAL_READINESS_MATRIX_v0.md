...
| COL-001..005 sink/batch scaffold only (not product collector) | **COL-001-SINK-MVP** + **COL-002-LIFECYCLE-MVP** + **COL-003-SEQ-MVP** + **COL-004-FAST-PATH-MVP** + **COL-005-BATCH-MVP** + **TEST-003-FAKE-CLOCK-MVP** board **done** (scaffold) | Overlay `collector/` semantic sink + lifecycle + gapless seq + **bounded batch + no-alloc stmt fast path** + fake-clock/M4 **mini** harness + unit tests + offline_gate step 10. **Residuals:** no live Perl/XS hooks; stub v5 does **not** write wire bytes (**COL-006**); **full M4 oracle corpus** v5-via-sink under fake-clock **not** claimed (needs COL-006 + complete TEST-003); COL-015 full fork/signal/**file-switch**/enable-disable matrix open (OI-003-05); COL-003 internal logical seq excludes `START_DEFLATE` — does **not** close **OI-001-03** dump dual-index; flush/compression **discount timing** vs BASE-003 open; light microbench **not** BENCH certification; no dual/v6 product sink. Evidence: [`collector/README.md`](https://github.com/hilather/nytprof-modernization/blob/main/collector/README.md), [`docs/schemas/collector-batch-fast-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/collector-batch-fast-mvp-v0.md), `scripts/packaging/collector_sink_smoke.sh` | R2 runway scaffolding; **not** a claim that collection is modernized in-process
...
| `COL-005-BATCH-MVP` | **done (scaffold)** | Bounded event batch + arena; order under cap 1..64; SV lifetime; emergency oversized; **not** COL-006 wire / flush-discount freeze |
| `TEST-003-FAKE-CLOCK-MVP` | **done (scaffold)** | Fake-clock + M4 **mini** sample; full corpus residual until COL-006 + complete TEST-003 |
