...
| `DROP-IN-REMAINING` | **residual** — opcode/full TEST-003/S2/COMPAT-007/publish. **DI-01 780/810**, **DI-02 27+CORE:**, **DI-04 mini projected kinds** landed. **DI-03 opcode/`entersub` — in progress, not done** (E1b default `OP_ENTERSUB`; **E2** `OP_GOTO`; **E3** opt-in `leave=1`, default 0; **E4** default `slowops=2` is the full 6.15 table (`full`/`=3` aliases); wrap escape `wrap=1` / `use_db_sub=1` / `entersub=0`; `SUB_CALLERS` C-aggregated at finish; live di02 **21** vs oracle **27**). **Not** DI-05 / GA-candidate / certified perf. |
...
Do **not** mark EL8 RPM or `BUILD-003-FULL` as ready. `CPAN-TRIAL-READY` is **notes-ready MVP** only (**not** PAUSE uploaded). G04 **attach-MVP** is landed (live `-d:NYTProf` default-calls1 **15/3/15**; E1b default call attach is opcode `OP_ENTERSUB` on the product sink; wrap is `wrap=1`). G03a **load** (no `nytprof.out` on trivial `-e`) remains; G03b–G03e emit-MVP remain. **Residuals:** mid-deflate continue-in-child, full TEST-018. DI-01 live **780/810** is landed. **DI-03 opcode/`entersub` — in progress, not done** (E1b default omit installs `OP_ENTERSUB`, emit after INIT, `$^P` 0x01 off; **E2** `OP_GOTO`; **E3** opt-in `leave=1`, default **0**; **E4** default `slowops=2` is the full 6.15 table (`full`/`=3` aliases); wrap escape `wrap=1` / `use_db_sub=1` / `entersub=0`; live di02 **21** vs oracle **27**). **Not** DI-05 / full TEST-003 / GA-candidate / certified perf. G05 options/`format=v6` and G06 fork/`addpid` MVP landed. Annex: [`product-xs-graft-annex-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/product-xs-graft-annex-v0.md). Dual-path remains **oracle-primary** until S2 ([BUILD_SUPPORT_POLICY](https://github.com/hilather/nytprof-modernization/blob/main/docs/BUILD_SUPPORT_POLICY.md)). `collection_default` stays **v5**.
...
| `PRODUCT-DBSTATE-TIMELINE-MVP` | **done (MVP)** | PR-15: default `stmts=1` TIME_LINE from C `OP_DBSTATE`; `$DB::single=0`. Close-then-seed last-site; sub incl/excl subtract `product_overhead_ticks`. Smoke [`g15_dbstate_timeline_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g15_dbstate_timeline_smoke.sh). Default calls are E1b opcode; wrap remains `wrap=1` only. **Not** NEXTSTATE / leave default 1. |
...
| `PRODUCT-SLOWOPS-FULL-OPT-IN` | **done (MVP / default)** | DI-03 E4 + 2026-08-18 flip: copied pin `slowops.h` on omit/`slowops=2` (same as `full`/`=3`). `slowops=1` still fail-closed. Smoke [`g19_slowops_full_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g19_slowops_full_smoke.sh). Thin emit + `product_credit_child_excl` — **not** 6.15 savestack exclusive (`sort`/backtick/`(?{ })` can double-count parent excl). |
