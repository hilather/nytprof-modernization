...
| COL-001..003 sink scaffold only (not product collector) | **COL-001-SINK-MVP** + **COL-002-LIFECYCLE-MVP** + **COL-003-SEQ-MVP** + **TEST-003-FAKE-CLOCK-MVP** board **done** (scaffold) | Overlay `collector/` semantic sink + lifecycle state machine + internal gapless seq + fake-clock/M4 **mini** harness + unit tests + offline_gate step 10. **Residuals:** no live Perl/XS hooks; stub v5 does **not** write wire bytes (**COL-006**); **full M4 oracle corpus** v5-via-sink under fake-clock **not** claimed (needs COL-006 + complete TEST-003); COL-015 full fork/signal/**file-switch**/enable-disable matrix open (OI-003-05); COL-003 internal logical seq excludes `START_DEFLATE` — does **not** close **OI-001-03** dump dual-index; no dual/v6 product sink. Evidence: [`collector/README.md`](https://github.com/hilather/nytprof-modernization/blob/main/collector/README.md), [`docs/schemas/collector-lifecycle-seq-fake-clock-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/collector-lifecycle-seq-fake-clock-mvp-v0.md), `scripts/packaging/collector_sink_smoke.sh` | R2 runway scaffolding; **not** a claim that collection is modernized in-process |
