...
- a performance certification (see residual row; **public claims waived** — R1-scoped P3/P4 methodology + light bench only; [`docs/BENCH_NOTES.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/BENCH_NOTES.md));
...
| R1 P3/P4 methodology (no public claims) | [`docs/BENCH_NOTES.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/BENCH_NOTES.md) + [`tools/bench/light_bench.sh`](https://github.com/hilather/nytprof-modernization/blob/main/tools/bench/light_bench.sh) |
...
| No full nytprofhtml DOM / REPORT-001..020 | **REPORT-001..020**, BASE-005, **REPORT-HTML-RESIDUAL-INV**, **REPORT-HTML-SHARED-CSS** (MVP `style.css` **done** — partial residual vs oracle CSS) | Native HTML is MVP summary + multi-file site (incl. shared `style.css` / inline CSS policy); not oracle DOM/tablesorter/JS/flame/Graphviz | REPORT_SURFACE_CONTRACT **not advertised** list; **artifact residual matrix:** [`docs/contracts/REPORT_HTML_RESIDUAL_INVENTORY_v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/contracts/REPORT_HTML_RESIDUAL_INVENTORY_v0.md); CSS structure [`docs/schemas/html-shared-css-structure-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/html-shared-css-structure-mvp-v0.md) |
...
| COL-001 sink scaffold only (not product collector) | **COL-001-SINK-MVP** board **done** (scaffold) | Overlay `collector/` semantic sink API + counting + stub v5 adapter + unit tests + offline_gate step 10. **Residuals:** no live Perl/XS hooks; stub v5 does **not** write wire bytes (COL-006); no lifecycle freeze (COL-002); no seq numbers (COL-003); no fake-clock (TEST-003 / PR-B03); no dual/v6 product sink; isolation asserts only (not oracle stream equality yet). Evidence: [`collector/README.md`](https://github.com/hilather/nytprof-modernization/blob/main/collector/README.md), [`docs/schemas/collector-sink-api-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/collector-sink-api-mvp-v0.md), `scripts/packaging/collector_sink_smoke.sh` | R2 runway scaffolding; **not** a claim that collection is modernized in-process |
...
| No performance certification claims | WP-13 / BENCH-001+; board **R1-P3P4-METHODOLOGY** (PR-A09) | Plan R1 wants controlled-host **P3** (native read/model) / **P4** (native report) results; full harness + thresholds not ratified | **Public claims waived** for R0 / R1-preview. Methodology + light harness only: [`docs/BENCH_NOTES.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/BENCH_NOTES.md), [`tools/bench/light_bench.sh`](https://github.com/hilather/nytprof-modernization/blob/main/tools/bench/light_bench.sh) (P3 proxies: dump/verify; P4 proxies: report/csv/html). **Not** BENCH-001 noise bounds, CI perf gates, or signed certificate. Revisit only if publishing numbers. |
...
| **Not claimed** | Multi-OS CI green matrix, CPAN upload, **performance SLOs / P3–P4 certification**, v6 collection, FFI ABI stability |
...
| `COL-001-SINK-MVP` | **done (scaffold)** | Semantic sink under `collector/` (ADR-0004); counting + stub v5; `make -C collector test`; smoke + offline_gate step 10. **Not** COL-006 wire / COL-007 / live hooks. |
