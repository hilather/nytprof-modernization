...
| `PRODUCT-SLOWOPS-FULL-OPT-IN` | **done (MVP / opt-in)** | DI-03 E4: copied pin `slowops.h` behind `slowops=full` / `=3`. Product `slowops=2` stays PRINT/MATCH (KD-35). `slowops=1` still fail-closed. Smoke [`g19_slowops_full_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g19_slowops_full_smoke.sh). Thin emit + `product_credit_child_excl` — **not** 6.15 savestack exclusive (`sort`/backtick/`(?{ })` can double-count parent excl on `=full` only). **Not** a default-on full table. |
