...
| CLI E5 v6 opt-in surfaces | **CLI-E5-V6-OPT-IN-MVP** **done** (PR-B12) | report/html/csv/folded/callgrind/dump/verify on v6; capability `v6_decode`/`v6_report` true; **`collection_default: v5`** (no default flip); tests `cli_e5_v6` + capability; schema [`docs/schemas/cli-e5-v6-opt-in-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/cli-e5-v6-opt-in-mvp-v0.md). **History:** PR-B12 shipped `convert`/`merge` **false**; **superseded on this branch by PR-C01/C02 — capability `convert`/`merge`/`repack`/`salvage` true**. | **Not** collection default flip (R4); convert/merge residual is **lossy/packing only** (tools done) |
...
| Convert / merge / salvage tooling | plan TOOL convert/merge; **PR-C01/C02 done** | **done** on this branch — capability `convert`/`merge`/`repack`/`salvage` **true**. Residual: lossy convert modes / packing fidelity only. | Honest **done** (PR-C01/C02; lossy residual) |
...
| Default engine/format flips | charter R3/R4 | Explicitly out of first slice and out of R2-preview | Native remains opt-in; `collection_default: v5` |
...
Historical R2-preview cut (PR-B13) residuals. On **this integrated R2-stable branch** (PR-C01–C05), convert/merge/salvage, COL-015 MVP, SEC-FUZZ offline, and P1/P2 methodology are **done** — see R2-stable section. Rows below remain as the PR-B13 snapshot context; strike-through means closed later on this branch.

| Residual | Why residual under R2-preview (PR-B13) | On this R2-stable branch |
|----------|----------------------------------------|--------------------------|
| ~~**No convert/merge/salvage claim**~~ | At PR-B13, capability stayed false until Phase C tools + gates | **Closed** by PR-C01/C02 — capability true; lossy residual only |
| ~~**No R2-stable claim**~~ | Fork suite, security/fuzz, P1/P2 public cert, convert tooling open at preview | **R2-stable honesty cut** PR-C05 (public perf still **waived**) |
| **No COL-008** | Deferred non-baseline (ADR-0007) | still deferred |
| **No R3/R4 defaults** | engine/format product default flips out of scope | still not claimed |
| **E3-mixed residual** | SOURCE/INDEX/SUMMARY product C matrix open | still residual |
| **Full oracle E4 residual** | TEST-008 class | still residual |
| **Lossy convert / packing** | (not listed at PR-B13 as primary residual) | explicit residual after PR-C01/C02 |
...
8. R2-preview packaging honesty (`R2-PREVIEW-READINESS-CUT`, release notes, COL-009 ADR-0007) historically listed convert/merge residual at PR-B13; on this integrated branch convert/merge/salvage are **done** (PR-C01/C02; lossy residual only). R2-stable cut PR-C05; no R3/R4 claims.
...
| `CLI-E5-V6-OPT-IN-MVP` | **done** (PR-B12) | Full offline CLI product surfaces on v6 (report/html/csv/folded/callgrind/dump/verify); capability honesty `v6_decode`/`v6_report` true, `collection_default: v5`; tests `cli_e5_v6` + capability_selftest; schema [`docs/schemas/cli-e5-v6-opt-in-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/cli-e5-v6-opt-in-mvp-v0.md). **History:** PR-B12 shipped convert/merge false; **superseded by PR-C01/C02 — capability convert/merge/repack/salvage true**. **Residuals:** collection default flip (R4); lossy convert/packing. E4 product: **E4-PRODUCT-CLI-SMOKE-MVP**. |
