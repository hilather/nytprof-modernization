...
| `DROP-IN-REMAINING` | **residual** — opcode/full TEST-003/S2/COMPAT-007/publish. **DI-01 780/810**, **DI-02 27+CORE:**, **DI-04 mini projected kinds** landed. **DI-03 opcode/`entersub` — in progress, not done** (E1b default omit installs `OP_ENTERSUB`; **E2 landed** `OP_GOTO` on default; wrap list still `wrap=1` only; `wrap=1` / `use_db_sub=1` / `entersub=0` escape; E3 leave / E4 full slowops residual). |
