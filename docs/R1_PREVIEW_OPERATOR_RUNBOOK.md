...
GHA matrix rows: **ubuntu-latest** (`linux-x86_64`) + **macos-latest** (`macos`). Honest skips inside `offline_gate.sh` are unchanged. This is **not** full multi-Perl / multi-rustc / Windows certification.
...
| **No full multi-OS CI matrix** | **BUILD-006-MVP** lands GHA **ubuntu-latest** + **macos-latest** via `./scripts/ci/matrix_gate.sh` (host-local oracle + same offline_gate; honest skips preserved). Full multi-Perl / multi-rustc / Windows / coverage dashboard / product multi-OS certification remains **BUILD-006** residual. |
