...
| 10 | `./scripts/packaging/collector_sink_smoke.sh` | **COL-001..003 + fake-clock scaffold** — isolation asserts always; `make -C collector test` (sink + lifecycle/seq + M4 mini) when CC present; **honest skip** without C toolchain. Stub v5 is **not** wire encode / COL-007; full M4 oracle residual |
