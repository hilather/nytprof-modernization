...
- full oracle **`nytprofhtml` DOM** / CSS / tablesorter / `flamegraph.pl` / Graphviz parity (native has opt-in `html --flame` folded SVG only; default off);
...
| **No full nytprofhtml DOM** | Native HTML is MVP summary / multi-file site (+ optional `--flame` folded SVG, default off) — not oracle DOM, CSS/JS, tablesorter, `flamegraph.pl`, Graphviz. See HTML residual inventory. |
