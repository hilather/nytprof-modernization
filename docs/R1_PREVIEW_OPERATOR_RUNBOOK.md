...
- full oracle **`nytprofhtml` DOM** / tablesorter / flame / Graphviz parity (A01/A02 MVP HTML only; A03 flame open; WAIVE classes residual);
...
| [R3_FIELD_WINDOW.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/R3_FIELD_WINDOW.md) | R3 field-window evidence pack (PR-D01; **no** default flip) |
...
| **R3/R4 defaults** | **OUT-OF-R1** | product default engine/format flip |
| **R3 field-window pack (PR-D01)** | **instrumentation only** — collect local evidence packs; **does not** flip defaults | R3 product default without field report + ADR (PR-D02) |

Release notes: [RELEASE_NOTES_R1.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/RELEASE_NOTES_R1.md).

### 7.1.1 R3 field window (post-cut; no default flip)

After the full R1 MVP cut, operators may collect **local** field evidence for a future `engine=auto` product policy decision **without** changing defaults:
...
**Not** charter R3 completion; promotion is a separate default-change ADR after an accepted field report.
...
| **No product default flip** | Native remains opt-in; Perl `engine=auto` is facade behavior, not charter R3 product default. Field packs ([R3_FIELD_WINDOW.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/R3_FIELD_WINDOW.md)) are evidence-only (`no_default_flip=true`). |
