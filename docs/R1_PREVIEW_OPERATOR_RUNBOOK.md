...
| `DI-03` opcode / `entersub` | **in progress, not done** — E1a opt-in: `NYTPROF=…:entersub=1` installs `OP_ENTERSUB` (emit after INIT). Default omit is still wrap + C `OP_DBSTATE`. `wrap=1` / `use_db_sub=1` still wins. E1b default flip / E2 `OP_GOTO` / di02 **27** residual. Plan: [DI03_OPCODE_ENTERSUB_ATTACH_v0.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/plan/DI03_OPCODE_ENTERSUB_ATTACH_v0.md) |
| Product `use_db_sub=1` | **Intentional fork** of the 6.15 option. On product it is a **synonym for `wrap=1`** (Perl `DB::sub` wrap escape). It is **not** 6.15 stmt `DB::DB` (`init_profiler` `if (opt_use_db_sub)` skips NEXTSTATE/DBSTATE while opcode `ENTERSUB` stays on). Do not copy that 6.15 meaning. `wrap=1` wins over `entersub=1`. |
