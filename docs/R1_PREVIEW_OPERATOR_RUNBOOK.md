...
| `PRODUCT-XS-ATTACH-MVP` / collection attach | **done (MVP)** — [g04_v5_parity_smoke.sh](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g04_v5_parity_smoke.sh) leaf **15** / mid **3** / edge **15**; E1b default `OP_ENTERSUB` (`$^P` 0x01 off); **E2** `OP_GOTO` ([g18_goto_sub_smoke.sh](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g18_goto_sub_smoke.sh)); wrap remains `wrap=1`. **Not** leave default 1. Default `slowops=2` is the 6.15 full table. |
| `DI-03` opcode / `entersub` | **in progress, not done** — E1b default: omit `entersub` installs `OP_ENTERSUB` (emit after INIT; `$^P` 0x01 off; `DB::sub` stub). **E2:** `OP_GOTO`. **E3:** opt-in `leave=1` (default 0). **E4:** default `slowops=2` is the full 6.15 table (`full`/`=3` aliases). Wrap list remains `wrap=1` / `use_db_sub=1` only. Escape / rollback: `NYTPROF=wrap=1`. Residual: live di02 **21** vs oracle start=begin **27**. **Not** DI-05 / GA-candidate / certified perf. Plan: [DI03_OPCODE_ENTERSUB_ATTACH_v0.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/plan/DI03_OPCODE_ENTERSUB_ATTACH_v0.md) |
