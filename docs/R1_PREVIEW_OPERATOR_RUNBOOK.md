...
| `PRODUCT-XS-ATTACH-MVP` / collection attach | **done (MVP)** — [g04_v5_parity_smoke.sh](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g04_v5_parity_smoke.sh) leaf **15** / mid **3** / edge **15**; E1b default `OP_ENTERSUB` (`$^P` 0x01 off); **E2 landed** `OP_GOTO` ([g18_goto_sub_smoke.sh](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g18_goto_sub_smoke.sh)). wrap remains `wrap=1`. **Not** E3 leave / E4 full slowops |
| `DI-03` opcode / `entersub` | **in progress, not done** — E1b default: omit `entersub` installs `OP_ENTERSUB` (emit after INIT; `$^P` 0x01 off; `DB::sub` stub). **E2 landed:** default also hooks `OP_GOTO` (`goto &sub` keeps original caller + goto-site fid:line). Wrap list remains `wrap=1` / `use_db_sub=1` only. Escape: `wrap=1` / `use_db_sub=1` / `entersub=0`. E3 leave default 0 / E4 full slowops / di02 **27** residual. Plan: [DI03_OPCODE_ENTERSUB_ATTACH_v0.md](https://github.com/hilather/nytprof-modernization/blob/main/docs/plan/DI03_OPCODE_ENTERSUB_ATTACH_v0.md) |
