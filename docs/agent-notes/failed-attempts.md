...
|------|------|------|-------|----------------------|--------|
| 2026-08-15 | ci-rocky8-lab-before-tag-rpm | ci / field | Required CI `rocky8-docker-lab` downloads `v*` testdrive RPM on the same tag push | Tag CI starts before `Release EL8 RPM` attaches the new NVR → 404 → red matrix. Smoke must SKIP when RPM is not on disk and not on the GitHub Release yet. Do not make the lab job require the RPM it is publishing. | — |
