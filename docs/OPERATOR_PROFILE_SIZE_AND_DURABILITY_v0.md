...
| Full 6.15 opcode / `entersub` / default-on `slowops.h` | Default `slowops=2` now installs the full `slowops.h` table (2026-08-18 flip). Exclusive remains thin (not 6.15 savestack). |
...
2. **Live attach** (`perl -d:NYTProfM`, `NYTPROF=...:aggregate=1`): workload-calls1 still has leaf/mid **returns** 15/3. Mid→leaf edge **count field** sums to 15 — parse `SUB_CALLERS.count` (or `nytprof-cli dump` / report JSON). Default attach already finish-flushes one `c` per distinct edge; [`t/installed_attach.t`](https://github.com/hilather/nytprof-modernization/blob/main/t/installed_attach.t) **sums `count`**. `aggregate=1` (this ADR) stays fail-closed. di01 `line_calls=780` is **TIME_BLOCK event occupancy**, not ticks — never run di01 with `aggregate=1` as that bar.
