# R4 field window — `format=v6` evidence (no default flip)

**Status:** instrumentation + report package only (**PR-E01**)  
...
**Does not:** flip product defaults, ship a default-change ADR (REL-008 / ADR-Q025), claim R4 complete, transmit telemetry, or change offline_gate defaults
...
This package is the **field-window half** of Track E (default-format evaluation). It **mirrors** the R3 field-window package structure ([`docs/R3_FIELD_WINDOW.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/R3_FIELD_WINDOW.md) when present on a branch that includes PR-D01) but targets **format**, not engine selection.

| Piece | Role | This PR |
|-------|------|---------|
| Evidence collector | Local pack under an output directory | **yes** — [`scripts/field/r4_field_window_collect.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/field/r4_field_window_collect.sh) |
| Smoke | Fixture-backed check that the collector works | **yes** — [`scripts/field/r4_field_window_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/field/r4_field_window_smoke.sh) |
| Report template | Human pack for multi-site review | **yes** — [`docs/templates/R4_FIELD_WINDOW_REPORT.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/templates/R4_FIELD_WINDOW_REPORT.md) |
| Pack schema | Layout + machine-readable summary | **yes** — [`docs/schemas/r4-field-window-mvp-v0.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/schemas/r4-field-window-mvp-v0.md) |
| Default-change ADR + product flip | REL-008 / ADR-Q025 (future PR) | **no** |

**Binding non-claims**

- Product **collection default remains v5** (`capability.collection_default: "v5"`). Packs must record `no_default_flip: true`.
...
These are **engineering defaults for the report**, not a ratified REL-001 policy freeze (ADR-Q025):
...
| Rollback bar | Document one-step default rollback plan for a future ADR (`collection_default` back to v5) while retaining v6 **read** |

Promotion itself is a **separate default-change ADR** (ADR-Q025 / REL-008), not this package.
...
| Claim | Status under PR-E01 |
|-------|---------------------|
| Field evidence **collection** tools + report template | **ready** |
| R4 product format default flip | **not** done — remains charter R4 residual |
| Default-change ADR (ADR-Q025 / REL-008) | **not** this PR |
...
Residual matrix: collection / format default flip remains **not claimed** with pointer to this pack as **instrumentation only**.

---

## 7. Exit criteria for the *window* (feeds default-change ADR — not auto-promoted)
...
Only then may a future PR draft a default-format ADR. **This package never flips defaults.**
