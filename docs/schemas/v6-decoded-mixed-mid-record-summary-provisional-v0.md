# Format v6 mid-record SUMMARY span on always-inflate multi-kind mixed path — provisional v0

**Status:** provisional — **not** a v6 wire freeze (not FMT-002..010 ratification; not COL-007 C writer)  
**Board IDs:** `FMT-V6-DECODED-MIXED-MID-RECORD-SUMMARY-PROVISIONAL` (contract), `FMT-V6-DECODED-MIXED-MID-RECORD-SUMMARY-MVP` (shipped encode/decode + tests)  
**Depends on:** `FMT-V6-DECODED-MIXED-MID-RECORD-INDEX-*`; `split_summary_body_bytes`; payload codecs NONE/ZLIB/ZSTD/LZ4; optional CRC  
...
This document freezes a **provisional SUMMARY mid-record-spanning consumer path** on the always-inflate multi-kind mixed stack:

```text
wire bytes
  → encode: co-present EVENT (full) + encode_summary_body mid-record split
             into ≥2 SUMMARY chunks under NONE/ZLIB/ZSTD/LZ4
...
- **SUMMARY** body bytes are split mid-record across ≥2 SUMMARY chunk payloads (interior split: neither piece alone is a complete multi-record SUMMARY body).
...
- EVENT/SOURCE/INDEX mid-record-on-mixed remain prior preflights.
...
| Condition | Result |
|-----------|--------|
| Truncated mid-record SUMMARY join | **Err** (summary-body layer) |
| CRC mismatch when verify on | **Err** |
| Corrupt compressed payload on span-carrying SUMMARY | **Err** |
...
use nytprof_format_v6::decoded_mixed::{
    decode_decoded_mixed_profile, encode_decoded_mixed_mid_record_summary_profile,
};
use nytprof_format_v6::mid_record_span::{default_mid_body_split, split_summary_body_bytes};
use nytprof_format_v6::summary_body::encode_summary_body;
...
let wire = encode_decoded_mixed_mid_record_summary_profile(
    6, 0, 0, 0, 0, &[], codecs, &events, &[], &[], &summaries, split, None,
)?;
...
| Item | Status |
|------|--------|
| Standalone SUMMARY mid-record | prior `FMT-V6-MID-RECORD-SUMMARY-*` |
| EVENT/SOURCE/INDEX mid-record on mixed path | prior `FMT-V6-DECODED-MIXED-MID-RECORD-*` siblings |
| SUMMARY mid-record on always-inflate multi-kind path | **done** (`FMT-V6-DECODED-MIXED-MID-RECORD-SUMMARY-*`) |
