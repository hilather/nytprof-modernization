...
This document freezes a **provisional** on-wire **chunk frame** (header + bounded payload length) so Rust (and later C) can share fail-closed framing before payload decode.

It is **not**:

- a permanent wire freeze or ADR-ratified FMT-002;
- permission to mark **COL-007** (C v6 writer) done;
- payload inflate/decode (zlib/zstd/LZ4 body), event opcodes, dictionaries, or location deltas;
- a claim that shipped CLI report/dump reads v6 chunks by default.

Sync word, kind/codec enums, and field widths may change under future ADR + immutable vectors.
