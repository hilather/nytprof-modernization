...
5. Heat classes are **`heat-hot` / `heat-high` / `heat-mid` / `heat-low`** only (quartile rank). Applied on **numeric cells** (`td.num.heat-*`); `tr.heat-*` remains as a fallback. Unused / zero source rows have **no** heat class. **Not** oracle `.c0`–`.c3` class names (CSS variables `--nyt-c0`…`--nyt-c3` hold the oracle palette; light values `#ffb3b3`…`#b4ffb4`, re-tuned to darker hues under dark mode).  
5a. **(2026-08-15 visual refresh)** The stylesheet ships a `prefers-color-scheme: dark` media block that overrides only `--nyt-*` variables; selectors, class names, and markup are unchanged. Additional tokens (`--nyt-surface`, `--nyt-row-hover`, `--nyt-target`, `--nyt-th-fg`, `--nyt-radius`, `--nyt-shadow`) are presentation-only. Carded tables (hairline separators, sticky `thead th`), pill `← Index`, `tr:target` highlight, and sort-affordance `⇅` hints are CSS-only; they do **not** change any structural marker in this contract.  
