...
**Status:** **A.6 G02 landed** (`libnytp_sink_v5.a` + load-only `Devel::NYTProf::CollectorBootstrap`). **A.4 G03a load landed** (product `perl -d:NYTProf` loads; no `nytprof.out` on trivial `-e`). **A.3/A.4/A.5 G03b–G03e emit-MVP landed**. **G04 attach-MVP landed** (`NYTPROF file=` + Perl `DB::sub`/`DB::DB`; default-calls1 leaf **15** / mid **3** / mid→leaf **15**). **G05 options/`format=v6` landed** (D1-B fail-closed; D1-A `NYTPROF6`). **G06 fork/`addpid` landed** (`CORE::GLOBAL::fork` → `nytp_fork_*` + addpid child file). **Not** full TEST-018 / mid-deflate-in-child / full 6.15 opcode/`entersub`.  
...
| **G04** | **landed (attach-MVP)** — `NYTPROF file=` + Perl `DB::sub`/`DB::DB` emit `SUB_RETURN` / `SUB_CALLERS` / `TIME_LINE` via shipped `nytp_emit_*`; default-calls1 leaf **15** / mid **3** / mid→leaf **15** | **No** FileHandle dual writer. **Not** full 6.15 opcode/`entersub`. **Residuals:** G05/G06/blocks-780 |
...
**G04 attach-MVP:** `NYTPROF file=` enables the file sink + `$^P` 0x01/0x02/0x20. **PR-7:** do not set `$DB::single` at enable. **PR-15:** default `stmts=1` installs C `OP_DBSTATE` TIME_LINE and leaves `$DB::single=0`. Live `perl -d:NYTProf` on `fixtures/v5/default-calls1/workload.pl`. Smoke [`g04_v5_parity_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g04_v5_parity_smoke.sh). Getopt/Exporter compile: [`g07_getopt_compile_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g07_getopt_compile_smoke.sh). C TIME_LINE: [`g15_dbstate_timeline_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g15_dbstate_timeline_smoke.sh). C wrap: [`g16_wrap_enter_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g16_wrap_enter_smoke.sh). `$PRODUCT_XS_ATTACH=1` only when `file=` is set. **Not** G05/G06/full opcode / stock 6.15 XS.
