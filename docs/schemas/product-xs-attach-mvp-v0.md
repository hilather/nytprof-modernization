...
| C `OP_DBSTATE` TIME_LINE | **PR-15:** default `stmts=1` emits TIME_LINE from `pp_product_dbstate_line` + last-COP fid pointer cache. Do **not** hook NEXTSTATE/UNSTACK (that is `blocks=1`). `$DB::single=0`. Close-then-seed last-site clock; close-to-seed is `product_overhead_ticks` and is subtracted from opcode/wrap sub incl/excl. Smoke [`g15_dbstate_timeline_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g15_dbstate_timeline_smoke.sh) (line **and** `do_ifmod` incl/excl < 55% of profiled wall). **Not** NEXTSTATE / `TIME_BLOCK` (that is `blocks=1`). Call attach is E1b opcode. |
