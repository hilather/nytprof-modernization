...
| C wrap push/pop | **PR-16:** default instrumented wrap is `DB::wrap_push`/`wrap_pop` (COP pin past package-DB + fid + clock + pending-excl + `SUB_RETURN`/`SUB_CALLERS`). `NYTPROF_WRAP_SLOW=1` is the old Perl `caller(0)`+XSUB control. Smoke [`g16_wrap_enter_smoke.sh`](https://github.com/hilather/nytprof-modernization/blob/main/scripts/packaging/g16_wrap_enter_smoke.sh). **Not** DI-03 / stock 6.15 XS. |
| `ATTRIBUTE application` | **$0** at `file=` enable (6.15 `output_header`). HTML primary file / “Profile of …” must not pick `Config_heavy.pl`. |
| `DB::sub` | Default: `wrap_push`/`wrap_pop` → `nytp_emit_*`. Compile-time / goto list still `goto &$raw`. |
