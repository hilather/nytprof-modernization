...
**2026-08-18 catalog method-time pair (claim: none):** host-local oracle 6.15 vs in-tree NYTProfM on catalog labs (scanner 8s; html_tree 5s; ppi / mojo / TT / json_xs / dbi_sqlite / csv_xs 4s). Same driver and seconds; native `csv --subs` on both profiles. Native did **more passes** in the same wall on every app (scanner **3.35×**, others **1.03–1.47×**). Exclusive **shape** on scanner is now a remainder (`tokenize` 197ms of 5.19s; `scan_file` 178ms of 7.20s) — the Aug 15 grandchild leak is gone. Method rows that still looked “off” were attribution: **then-default** `slowops=2` was PRINT/MATCH only. **2026-08-18 flip:** product omit/`2` now installs the full 6.15 table so `CORE:open`/`readline`/`subst` appear by default. Thin `CORE:match` is still a leaf, so PPI `Lexer::CORE:match` remains **8.6×/pass** vs oracle (oracle parents `PPI::Node::content` under the opcode). XSUB-heavy `DBI::st::execute` / `Cpanel::JSON::XS::{en,de}code` are **1.07–1.25×/pass**. Not BENCH cert. Detail: [`docs/agent-notes/details/catalog-method-times.md`](https://github.com/hilather/nytprof-modernization/blob/main/docs/agent-notes/details/catalog-method-times.md).

**2026-08-17 DI-03 E1a (opt-in `entersub=1`, claim: none):** same-host g17 leaf-loop N=40000, `stmts=0`. Wrap default **0.366s**, product `entersub=1` **0.092s**, isolated 6.15 pin `-d:NYTProf` **0.058s**. Direction: opcode cheaper than wrap on this host; **not** a 6.15 beat and **not** BENCH certification. Command: `bash scripts/packaging/g17_entersub_attach_smoke.sh`. Default attach stays wrap.

**2026-08-17 DI-03 E1b (default opcode, claim: none):** same-host g17 leaf-loop N=40000, `stmts=0`. wrap=1 **0.405s**, default omit (opcode) **0.104s**, isolated 6.15 pin `-d:NYTProf` **0.084s**. Direction: default opcode cheaper than wrap=1 on this host; **not** a 6.15 beat and **not** BENCH certification. Command: `bash scripts/packaging/g17_entersub_attach_smoke.sh`. Rollback: `NYTPROF=wrap=1`.

**2026-08-17 SUB_CALLERS C aggregate (claim: none):** finish flush of distinct edges; `SUB_RETURN` still 1:1. Same host, inner `Time::HiRes`, `stmts=0`. N=120000 mean of 2: wrap=1 **0.877s** / default opcode **0.123s** / isolated 6.15 **0.160s**. N=40000 mean of 3: wrap **0.268s** / opcode **0.043s** / 6.15 **0.052s**. g17 process-wall N=40000: wrap **0.301s** / opcode **0.056s** / 6.15 **0.063s**. Direction: default-compress opcode cheaper than wrap and cheaper than 6.15 on this call loop; **not** BENCH cert. Command: `bash scripts/packaging/g20_callers_agg_smoke.sh` + same leaf/mid loop as g17. 25s paired scanner (`compare_oracle_native_reports.sh --seconds 25`, 2-file corpus): native **3468** passes / 24.6s Σ-ticks / **433 KB** `nytprof.out`; oracle **1162** passes / 9.30s of 24.9s / **5.45 MB**. Same wall and corpus; native did more app work (expected). Not a public perf claim.
