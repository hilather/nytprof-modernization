...
static void note_kind(nytp_sink *sink, counting_impl *ci, nytp_event_kind kind)
{
    ...
    if (nytp_event_kind_is_logical(kind)) {
        /* Sequence is committed by the public wrapper after this returns;
         * peek the in-flight next_seq as the assigned value. */
        ...
        if (ci->stats.seq_ring_len < NYTP_COUNTING_SEQ_RING) {
            ci->stats.seq_ring[ci->stats.seq_ring_len++] = s;
        } else {
            /* Drop oldest. */
            memmove(ci->stats.seq_ring, ci->stats.seq_ring + 1,
                    (NYTP_COUNTING_SEQ_RING - 1) * sizeof(nytp_seq));
            ci->stats.seq_ring[NYTP_COUNTING_SEQ_RING - 1] = s;
        }
    }
}
                                           ...
                                           nytp_string_view value)
{
    (void)key;
    (void)value;
    note_kind(sink, ci_of(sink), NYTP_EVT_ATTRIBUTE);
    return NYTP_OK;
}
                                        ...
                                        nytp_string_view value)
{
    (void)key;
    (void)value;
    note_kind(sink, ci_of(sink), NYTP_EVT_OPTION);
    return NYTP_OK;
}
...
{
    (void)text;
    note_kind(sink, ci_of(sink), NYTP_EVT_COMMENT);
    return NYTP_OK;
}
                                           ...
                                           nytp_fid fid, nytp_line line)
{
    ...
    note_kind(sink, ci, NYTP_EVT_TIME_LINE);
...
{
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_TIME_BLOCK);
    ci->stats.last_ticks = ticks;
    ci->stats.last_fid = fid;
    ci->stats.last_line = line;
...
{
    note_kind(sink, ci_of(sink), NYTP_EVT_DISCOUNT);
    return NYTP_OK;
}
                                         ...
                                         uint32_t mtime, nytp_string_view name)
{
    ...
    (void)name;
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_NEW_FID);
    ci->stats.last_fid = fid;
    return NYTP_OK;
}
                                          ...
                                          nytp_string_view text)
{
    (void)text;
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_SRC_LINE);
    ci->stats.last_fid = fid;
    ci->stats.last_line = line;
    return NYTP_OK;
}
...
{
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_SUB_INFO);
    ci->stats.last_fid = fid;
    ...
    nytp_string_view caller)
{
    ...
    (void)caller;
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_SUB_CALLERS);
    ci->stats.last_fid = fid;
    ci->stats.last_line = line;
                                           ...
                                           nytp_pid ppid, double start_time)
{
    ...
    (void)start_time;
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_PID_START);
                                         ...
                                         double end_time)
{
    (void)end_time;
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_PID_END);
    ci->stats.last_fid = (nytp_fid)pid;
    return NYTP_OK;
}
...
{
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_SUB_ENTRY);
                                            ...
                                            nytp_string_view subname)
{
    ...
    (void)excl_time;
    counting_impl *ci = ci_of(sink);
    note_kind(sink, ci, NYTP_EVT_SUB_RETURN);
...
{
    note_kind(sink, ci_of(sink), NYTP_EVT_START_DEFLATE);
    return NYTP_OK;
}
