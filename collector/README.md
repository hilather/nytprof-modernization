...
- **G03a debugger load + G03b–G03e emit-MVP + G04 attach-MVP + G05 options/`format=v6` + G06 fork/`addpid` are landed** (`perl -d:NYTProf` + `make -C collector xs-nytprof` / `xs-nytprof-v6` + `nytp_emit_*` + E1b default `OP_ENTERSUB` + `CORE::GLOBAL::fork` → `nytp_fork_*`). Wrap is `wrap=1`. G04 smoke default-calls1 leaf **15** / mid **3** / edge **15**. G05: D1-B `format=v6` fail-closed; D1-A writes `NYTPROF6`. G06: parent + `<file>.<pid>` `NYTProf 5`. **Not** full TEST-018 / mid-deflate-in-child / DI-03 done (E2 GOTO / E3 leave / E4 full slowops). G02 CollectorBootstrap remains a distinct load-only module.  
